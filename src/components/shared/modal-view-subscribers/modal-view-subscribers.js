...
const ModalViewSubscribers = () => {
    ...
    const { personId } = useProfileById();
    const { myId } = useProfileStore()

    useEffect(() => {
        getMySubscribers(personId);
            ...
            >
                <div className='sticky top-0 modal-theme z-50'>
                    <div className='relative flex justify-center border-b-[1px] border-gray-500/40 py-2'>
                        ...
                        <IconButton onClick={setCloseModalViewSubscribers} sx={{ position: 'absolute', top: '0', right: '0', color: 'currentColor' }}><Close /></IconButton>
                    </div>
                ...
                <div className='flex flex-col'>{mySubscribers.length > 0 && mySubscribers?.filter((person) => person.userShortInfo.userName.trim().toLowerCase().includes(searchValue.trim().toLowerCase())).map((person, i) =>
                    <div
                        ...
                        <div className='flex items-center gap-3 justify-start'>
                            ...
                            <div>
                                <div className='flex gap-2 items-center'>
                                    ...
                                    {(personId == myId) == true &&
                                        <Button onClick={() => {
                                            subscribe(person?.userShortInfo?.userId)
                                            setStateDetector()
                                        }} sx={{ textTransform: 'none', padding: '0 10px', fontSize: '13px' }}>Подписатья</Button>
                                    }
                                </div>
                        ...
                        {(personId == myId) == true &&
                            <Button onClick={() => {
                                deleteMySubscriber(person?.userShortInfo?.userId)
                                setStateDetector()
                            }} variant='contained' sx={{ padding: '3px 10px', textTransform: 'none', backgroundColor: '#1571', color: dm ? 'white' : 'black', borderRadius: '10px', fontSize: '14px' }}>Удалить</Button>
                        }
