...
const ModalViewSavedPost = ({ savedPerson }) => {
    ...
    const [showHeartForIt, setShowHeartForIt] = useState(null);
    ...
    const {
        isOpenModalViewSavedPost,
        initialSlide,
        setCloseModalViewSavedPost,
        initialSlideOfSavedPost,
        getPersonPosts,
        personPosts,
        person,
        inputs,
    ...
    const { personPostsById, inputsById, likePostById, addCommentToPostById, setInputById, getUsers, getPersonById, personById, personId, getPersonPostsById, initialSlideById } = useProfileById()
    ...
    return (
        <>
            ...
            <ModalDeleteComment />
            <Modal
                open={isOpenModalViewSavedPost}
                onClose={setCloseModalViewSavedPost}
                aria-labelledby='modal-modal-delete'
            >
                <div
                    className={'w-[100%] outline-none absolute translate-x-[-50%] overflow-auto translate-y-[-50%] left-[50%] top-[50%] flex justify-between '.concat(bp.w767 ? 'h-full p-0 flex-col' : 'h-[90vh] p-2 items-center gap-4')}
                    onClick={setCloseModalViewSavedPost}
                >
                    {bp.w767 &&
                        <div onClick={setCloseModalViewSavedPost} className={` ${dm ? 'bg-[#111]' : 'bg-white'} cursor-pointer sticky top-0 z-50 p-1 w-full`}>
                    ...
                    >
                        <Swiper
                            ...
                            pagination={false}
                            modules={[Navigation]}
                            initialSlide={initialSlideOfSavedPost}
                            allowTouchMove={false}
                            className={`mySwiper ${bp.w767 ? '' : 'rounded'} ${dm ? 'bg-[#111]' : 'white'} size-[100%]`}
                        >
                            {personSaved.map((post, i) => (
                                <SwiperSlide className={`${dm ? 'bg-[#111]' : 'white'} overflow-y-auto`} key={i}>
                                    <div
                                    ...
                                    >
                                        ...
                                        >
                                            {post.images.map((item, index) => (
                                                <SwiperSlide className={`${dm ? 'bg-[#111]' : 'white'} overflow-y-auto`} key={index}>
                                                    <div
                                                        onDoubleClick={(e) => {
                                                            handleDoubleClick(e)
                                                            setShowHeartForIt(index)
                                                            likePostById(post.postId);
                                                            setStateDetector()
                                                        }}
                                                        className='h-full relative bg-gray-950 w-full'>
                                                        {showHeart && showHeartForIt == index && (
                                                            <div
                                                                className="heart-container"
                                                                style={{
                                                                    position: "absolute",
                                                                    left: `${heartPosition.x + 20}px`,
                                                                    top: `${heartPosition.y - 100}px`,
                                        ...
                                        <div style={{ scrollbarWidth: 'none' }} className='flex flex-col overflow-auto justify-between'>
                                            <div className=''>
                                                            ...
                                                            >
                                                                <div className='flex gap-4 items-center'>
                                                                    ...
                                                                    <div className='flex flex-col items-start'>
                                                                        <p className='font-[500] text-[13px]'>
                                                                            <span className='text-[15px] font-[600] mr-2'>
                                                                                {comment?.userName}
                                                                            </span>
                                                                            {comment.comment}
                                                                        </p>
                                                ...
                                                {i == initialSlideOfSavedPost && <SwipableDrawer isOpen={isOpenDrawer} onClose={() => setIsOpenDrawer(false)}>
                                                        ...
                                                        <div className={`px-2 gap-2 absolute bottom-0 border-t-[1px] ${dm ? 'bg-[#111]' : 'bg-white'} border-gray-500 w-full items-center justify-between flex`}>
                                                            <div>
                                                                {currentPostId == post.postId && <Menu
                                                                    ...
                                                                    sx={{ maxWidth: '500px', top: '-45px', right: '-155px', width: '100%', padding: '0' }}
                                                                    onClose={handleClose}
                                                                ...
                                                                >
                                                                    ...
                                                                    <EmojiPicker onEmojiClick={(e) => {
                                                                        if (currentPostId) {
                                                                            setInputById(
                                                                                currentPostId,
                                                                                inputsById[currentPostId] ? inputsById[currentPostId] + e?.emoji : e?.emoji
                                                                            );
                                                                        }
                                                                    }} />
                                                                </Menu>}
                                                            ...
                                                            <input
                                                                ...
                                                                value={inputsById[post.postId] || ''}
                                                                onChange={e => {
                                                                    setInputById(post.postId, e.target.value)
                                                                }}
                                                                onKeyPress={e => {
                                                                    if (e.key == 'Enter') {
                                                                        addCommentToPostById({
                                                                            comment: inputsById[post.postId],
                                                                            postId: post.postId,
                                                                        })
                                                                        setStateDetector()
                                                                        setInputById(post.postId, '')
                                                                    }
                                                                }}
                                                            />
                                                            <IconButton
                                                                disabled={
                                                                    inputsById[post.postId]?.trim() ? false : true
                                                                }
                                                                onClick={() => {
                                                                    addCommentToPostById({
                                                                        comment: inputsById[post.postId],
                                                                ...
                                                                color='primary'
                                                            >
                                                                <SendRounded sx={{ color: !inputsById[post.postId]?.trim() ? 'gray' : '#59f' }} />
                                                            </IconButton>
                                                        </div>
                                                    </div>
                                                </SwipableDrawer>}
...
export default ModalViewSavedPost